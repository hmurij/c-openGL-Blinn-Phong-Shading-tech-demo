...
void main(void)
{
    ...
    vs_out.N_flat = vs_out.N = mat3(model) * normal;

    // Calculate light vector
    vs_out.L_flat = vs_out.L = light_pos - P.xyz;

    // Calculate view vector
    // vs_out.V = -P.xyz;
    vs_out.V_flat = vs_out.V = viewPos - (model * position).xyz;
...
void main(void)
{
    ...
    vec3 N = normalize(flat_color == true ? fs_in.N_flat : fs_in.N);
    vec3 L = normalize(flat_color == true ? fs_in.L_flat : fs_in.L);
    vec3 V = normalize(flat_color == true ? fs_in.V_flat : fs_in.V);
